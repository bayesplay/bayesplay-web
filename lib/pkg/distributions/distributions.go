...
func Integrate(f func(float64) float64, min float64, max float64) float64 {
	auc := quad.Fixed(f, min, max, 10000, nil, 100000)
	return auc
}
...
func ScaledShiftedT(x float64, mean float64, sd float64, df float64) float64 {
...
func Dt(x float64, df float64, ncp float64) float64 {
	...
	Px *= trm1 + trm2
  if math.IsNaN(Px) || df > 1e6 {
		// return 0
    return Dnorm(x, ncp, 1)
	}
	return Px
}

// func ScaledShiftedT(x float64, mean float64, sd float64, df float64) float64 {
//   return Dt((x - mean)/sd, df, 0.0)  / sd
// }
