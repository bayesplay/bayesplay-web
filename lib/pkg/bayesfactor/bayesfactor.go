...
func StudentTPrior(mean float64, sd float64, df float64, min float64, max float64) Prior {
	...
	if min == math.Inf(-1) && max == math.Inf(1) {
		...
		student_t := func(x float64) float64 {
			return ScaledShiftedT(x, mean, sd, df) * inrange(x, min, max)
		}
		auc := Integrate(student_t, math.Inf(-1), math.Inf(1))
		k := 1 / auc
		var prior Prior
		prior.Function = func(x float64) float64 {
			return (ScaledShiftedT(x, mean, sd, df) * inrange(x, min, max)) * k
		}
		prior.Name = "student_t"
		return prior
	}
